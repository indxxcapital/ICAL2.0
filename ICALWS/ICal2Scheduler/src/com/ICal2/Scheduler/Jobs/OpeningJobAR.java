...
public class OpeningJobAR implements Job{ 
	
	public void execute(JobExecutionContext context) throws JobExecutionException
	{
		System.out.println("Generation Opening Files for AR Zone  --->>> Time is " + new Date());
		generateOpeningFilesAR();
		
	} 
	
	public void generateOpeningFilesAR()
	{
		System.out.println("inside generateOpeningFilesAR");
		LocalDate localDate = LocalDate.now();
		String strDate = DateTimeFormatter.ofPattern("yyy-MM-dd").format(localDate);
		
		IndexService iservice = new IndexService();
		List<IndexBean> iList = iservice.getAllLiveIndexForFileGeneration("AR");
